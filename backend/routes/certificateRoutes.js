...
const upload = multer({
  ...
  fileFilter: (_, file, cb) => {
    ...
    if (!allowed.includes(file.mimetype)) return cb(new Error("Only PDF, PNG and JPEG files are allowed"));
    cb(null, true);
  }
});
...
router.patch("/:certificateId/revoke", controller.revokeCertificate);
